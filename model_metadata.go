...
func (store *modelMetadataStore) Decide(model string) AnonymousDecision {
	...
	// Once a model has an authoritative price record, that record wins over a
	// name-based guess. The name fallback remains available while metadata is
	// pending or the model is absent, but a known paid model must not enter the
	// anonymous lane merely because its ID contains "free".
	if metadataFree {
